...
use crate::{BoxState, BoxStore, MultinumBox, PolynumBox, RawBox, RawBoxOwned};

impl BoxStore {
    fn derivative_uni_raw(&self, poly: RawBox<'_, PolynumBox>) -> RawBoxOwned<PolynumBox> {
    ...
    pub fn derivate_uni(&self, state: BoxState<PolynumBox>) -> BoxState<PolynumBox> {
        let raw = state.as_raw(self);
        BoxState::Uncommitted(self.derivative_uni_raw(raw))
    }

    fn derivative_multi_raw(
        &self,
        multi: RawBox<'_, MultinumBox>,
        index: impl Into<Natural>,
    ...
    pub fn derivative_multi(
        &self,
        state: BoxState<MultinumBox>,
        index: impl Into<Natural>,
    ) -> BoxState<MultinumBox> {
        let raw = state.as_raw(self);
        BoxState::Uncommitted(self.derivative_multi_raw(raw, index))
    }
}
...
mod tests {
    ...
    use crate::{BoxState, BoxStore, Color, MultinumBox, PolynumBox};
    ...
    fn test_der_uni() {
        ...
        let alpha_3 = store.wrap_in_box::<PolynumBox>(&three, Color::Black, 1_u32);
